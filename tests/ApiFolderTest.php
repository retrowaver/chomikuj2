...
final class ApiFolderTest extends TestCase
{
	...
	public function testGetFoldersByUsernameCorrectlyReadsValidResponse(): void
	{
        ...
        $folders = $api->getFoldersByUsername();

        $this->assertTrue(is_array($folders));
        $this->assertEquals(2, count($folders));

        $this->assertEquals(1, $folders[1]['id']);
        $this->assertEquals('Dokumenty', $folders[1]['name']);
        $this->assertEquals('/username/Dokumenty', $folders[1]['path']);
	}
}